function validateForm(event) {
    // get a reference to the form. Because we attached a submit event listener to the form itself, 
    // we can access the form either through 'event.target', or 'this'
    const theForm = event.target;
    // the default behavior for a form submit is to try and navigate to another page where the form 
    // would be processed, if a url is not provided it will reload the current page. This sometimes 
    // is not desirable behavior. One case when we might do this is if we think there is bad data 
    // in the form.
    // To keep it from happening we can can call e.preventDefault()
    // You should always give feedback to the user about what whet wrong so they can fix it. 
    // We will store the error messages here
    const errors = [];
    ...
    if (theForm.paymentMethod.value === "creditCard") {
        if (!theForm.creditCardNumber.value.match(/^[0-9]{16}$/)) {
            isValid = false;
            errors.push("Please enter a valid 16-digit credit card number");
        }
    } else if (theForm.paymentMethod.value === "paypal") {
        if (theForm.paypalUsername.value.trim() === "") {
            isValid = false;
